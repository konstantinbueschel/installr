...
function listApps(id) {
	...
	var r = request.get({
		...
		var table = new Table({
			head:      [
				chalk.white('id'), chalk.white('AppId'), chalk.white('Platform'), chalk.white('Version'),
				chalk.white('buildId'), chalk.white('Installs')
			],
		...
		JSON.parse(body).appList.forEach(function (app) {
			...
			function writeRow() {
				table.push(
					[
						chalk.white(app.id), chalk.white(app.appId), chalk.white(app.type),
						app.latestBuild.versionNumber, app.latestBuild.id, chalk.white(app.latestBuild.numberInstalled)
					]
				);
			}
...
function uploadApp() {
	...
	var r = request.post({
		...
		if (err) {
			...
			var r = request.post({
				url:     'https://www.installrapp.com/apps/' + resp.appData.id + '/builds/'
						 + resp.appData.latestBuild.id + '/team.json',
				headers: {
					'X-InstallrAppToken': config.token
				}
...
function installrapp() {
	...
	if (program.upload) {
		...
		if (_.isArray(program.upload)) {

			program.upload = program.upload[0];
		}


		if (fs.existsSync(path.join("./", program.upload))) {

			console.log("Using local app file");
			config.filePath = "./" + program.upload;
		}
		else if (fs.existsSync(path.join("./dist/", program.upload))) {

			console.log("Using ./dist folder app file");
			config.filePath = "./dist/" + program.upload;
		}
		else if (fs.existsSync(path.join(''.concat(iTunesPath), program.upload))) {
